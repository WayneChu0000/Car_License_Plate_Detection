...
from main import run_realtime_plate_detection, resolve_model_path
...
class LicensePlateGUI:
    def __init__(self, root):
        ...
        # Apply native Windows styling (which natively provides tick instead of cross for checkboxes)
        style = ttk.Style(self.root)
        try:
            style.theme_use('vista')  # Best for modern Windows (10/11) native look
        except tk.TclError:
            pass # Fall back to the system's default
    ...
    def create_widgets(self):
        ...
        ttk.Label(main_frame, text="Confidence (0-1):", font=("Helvetica", 10)).grid(row=4, column=0, sticky=tk.NW, pady=5)
        ...
        self.conf_label = ttk.Label(main_frame, text="0.25")
        self.conf_label.grid(row=4, column=2, pady=5)
        ttk.Label(main_frame, text="Minimum certainty required to flag a license plate.", font=("Helvetica", 8, "italic"), foreground="gray").grid(row=5, column=1, sticky=tk.W, pady=(0, 10))
        ...
        ttk.Label(main_frame, text="Output Video FPS:", font=("Helvetica", 10)).grid(row=10, column=0, sticky=tk.W, pady=5)
        ...
        fps_combo.grid(row=10, column=1, sticky=tk.W, padx=5, pady=5)

        # 7. Checkboxes (Show FPS, Save Output)
        self.show_fps_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(main_frame, text="Show Current FPS Target Overlay", variable=self.show_fps_var, command=self.update_shared_config).grid(row=11, column=0, columnspan=2, sticky=tk.W, pady=5)

        self.save_video_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(main_frame, text="Save Output to realtime_detection_output.mp4", variable=self.save_video_var).grid(row=12, column=0, columnspan=3, sticky=tk.W, pady=5)

        # 8. Start/Stop Button
        self.start_btn = ttk.Button(main_frame, text="▶ Start Detection", command=self.start_detection)
        self.start_btn.grid(row=13, column=0, columnspan=3, pady=25, ipadx=10, ipady=10)
